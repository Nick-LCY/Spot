...
def init_environment_handler():
    ...
    manager.data.set("current_replicas", configs_obj["replicas"])
    manager.data.set("original_replicas", configs_obj["replicas"])
...
@register(event="start_frontend_replicas")
def handle_frontend():
    test_case = manager.data.get("current_test_case")
    assert isinstance(test_case, TestCase)
    update_replicas("frontend", test_case.additional["frontend_replicas"])


@register(event="start_search_replicas")
def handle_search():
    test_case = manager.data.get("current_test_case")
    assert isinstance(test_case, TestCase)
    update_replicas("search", test_case.additional["search_replicas"])


@register(event="start_reservation_replicas")
def handle_reservation():
    test_case = manager.data.get("current_test_case")
    assert isinstance(test_case, TestCase)
    update_replicas("reservation", test_case.additional["reservation_replicas"])


@register(event="start_profile_replicas")
def handle_profile():
    test_case = manager.data.get("current_test_case")
    configs_obj = manager.data.get("configs")
    deployer = manager.components.get("deployer")
    ...
    log.key("Restarting apps")
    deployer.restart(configs_obj["app"], configs_obj["port"])
    update_replicas("profile", test_case.additional["profile_replicas"])


def update_replicas(ms: str, count: int):
    deployer = manager.components.get("deployer")
    assert isinstance(deployer, BaseDeployer)
    replicas: dict = manager.data.get("current_replicas")
    replicas.update({ms: count})
    log.key(f"Updated replicas, {ms}: {count} pods")
    deployer.reload(replicas)
